...
% theta1_abs = pi/2 - q1;
% theta2_abs = q2- q1 - pi/2;
% theta3_abs = q3- q1 - pi/2;
theta1_abs = pi/2 + q1;
theta2_abs = q2 + q1 - pi/2;
theta3_abs = q3 + q1 - pi/2;

% hip
pMh = rot(theta1_abs) *[r;0];
% torso
pMt = pMh + rot(theta3_abs) *[l;0];
% stance leg
pm1 = rot(theta1_abs) *[r/2;0];
% swing leg
pm2 = pMh + rot(theta2_abs) *[r/2;0];
% center of mass
pcm = (pMh*Mh + pMt*Mt + pm1*m + pm2*m)/(Mh + Mt + m + m);
% end of swing leg
P2 = pMh + rot(theta2_abs) *[r;0];
...
for k = 1:N
    for j = 1:N
        C(k,j) = 0*g;
        for i = 1:N
            C(k,j) = .5*(  diff(D(k,j),q(i)) +  diff(D(k,i),q(j))  +  diff(D(i,j),q(k))  );
        end
    end
end
C = simplify(C);
...
K_Mhe =  .5*Mh*(vMh.'*vMh);
K_Mte = .5*Mt*(vMt.'*vMt);
K_m1e = .5*m*(vm1_e.'*vm1_e);
K_m2e = .5*m*(vm2_e.'*vm2_e);
